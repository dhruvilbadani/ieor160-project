...
def read_data(filename):
	...
	with open(filename) as f: # Opening the file containing the data (NOT necessarily .dat)
		...
		lines = lines[251:] # Skipping the first 251 lines since we need out-of-sample error and the first line is just feature names
...
print("SSE All 10: {0}".format(sse(coeffs_given, X, Y_actual))) # The SSE using  te coefficients given

coeffs_part_a = [0, -153.335, 610.97, 0, 0, 0, 0, 0, 588.782, 188.019] # The coefficients from part (a)
print("SSE Part A: {0}".format(sse(coeffs_part_a, X, Y_actual))) # The SSE using  te coefficients from part (a)

coeffs_part_b = [0, -155.447, 597.687, 213.927, 0, 0, 0, 0, 600.31, 0] # The coefficients from part (b)
print("SSE Part B: {0}".format(sse(coeffs_part_b, X, Y_actual))) # The SSE using  te coefficients from part (b)

coeffs_part_c = [0, 0, 405.494, 0.000540696, 0, 0, 0, 0, 414.866, 23.1162] # The coefficients from part (c)
print("SSE Part C: {0}".format(sse(coeffs_part_c, X, Y_actual))) # The SSE using  te coefficients from part (c)

coeffs_part_d = [0, 0, 603.681, 200.878, -267.699, 0, 0, 0, 711.644, 0] # The coefficients from part (d)
print("SSE Part D: {0}".format(sse(coeffs_part_d, X, Y_actual))) # The SSE using  te coefficients from part (d)
